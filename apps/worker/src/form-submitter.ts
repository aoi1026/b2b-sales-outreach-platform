...
async function getElementMeta(_page: Page, el: ElementHandle<Element>) {
  ...
  try {
    raw = await el.evaluate((node): RawMeta => {
      ...
      if (!labelText) {
        // 1セル内に複数入力が並ぶ (姓[input]名[input] / セイ[input]メイ[input]) レイアウト用:
        // 入力欄の直前の兄弟ノード (テキスト/小要素) を短いラベルとして拾う。
        // 例: reg26.smp.ne.jp は name/class/value に姓名情報が無く、隣接テキストが手掛かり。
        let sib: ChildNode | null = e.previousSibling;
        for (let k = 0; sib && k < 4 && !labelText; k++) {
          let t = "";
          if (sib.nodeType === 3) t = sib.textContent || "";
          else if (sib.nodeType === 1 && !(sib as Element).querySelector("input, select, textarea"))
            t = sib.textContent || "";
          t = t.replace(/[\s　]+/g, " ").replace(/[*＊※:：]|必須|任意/gi, "").trim();
...
export function detectFieldRole(meta: ElementMeta): FieldRole {
  ...
  // 漢字氏名がひらがな欄へ流し込まれる誤りを防ぐ。
  {
    ...
    // name/id が日本語ラベルそのもの (name="姓" / name="セイ" / name="めい" 等) のフォーム
    // (例: asagami.co.jp) に対応するため、idOrName のトークン一致でも姓/名を判定する。
    // idOrName は `${idLower}|${nameLower}` なので、`(?:^|\|)…(?:\||$)` で「丸ごと一致」を見る。
    // ※ 会社名(会社名)・氏名 等の部分一致を避けるため必ずトークン境界で限定する。
    const lastToken = /(?:^|\|)(?:姓|セイ|せい)(?:\||$)/.test(idOrName);
    const firstToken = /(?:^|\|)(?:名|メイ|めい)(?:\||$)/.test(idOrName);
    // name/id/placeholder に手掛かりが無く、ラベル (隣接テキスト/見出し) だけが姓名を表す
    // フォーム (例: reg26.smp.ne.jp) 用。ラベルの「先頭」一致に限定して会社名/お名前/氏名の
    // 誤検出を避ける (「姓」「名」「セイ」「メイ」「せい」「めい」で始まる短いラベルのみ採用)。
    ...
    if (partLast || partFirst) {
      ...
      const kata =
        ...
        (/katakana|furigana|gana|(?:^|[_\-])kana(?:[_\-]|$)|kana|フリガナ|カナ|カタカナ/.test(idOrName) ||
          /(?:^|\|)(?:セイ|メイ)(?:\||$)/.test(idOrName) || // name="セイ"/"メイ"
          /^(?:セイ|メイ)(?:\s|　|（|\(|:|：|$)/.test(lbl) || // ラベルが「セイ」「メイ」
...
async function findConfirmationSendButton(
  ...
  for (const el of candidates) {
    ...
    const text = tag === "button" ? ((await el.textContent()) ?? "").trim() : "";
    const hay = `${value} ${text}`;
    if (NEGATIVE_BUTTON_RE.test(hay)) continue; // 「戻る」等は除外
    if (CONFIRM_SUBMIT_TEXT_RE.test(hay)) return el;
  }
  return null;
}
...
async function findSendButtonByText(page: Page): Promise<ElementHandle<Element> | null> {
  ...
  for (const el of els) {
    ...
    if (CONFIRM_SUBMIT_TEXT_RE.test(hay)) {
...
async function clickConfirmationChain(
  ...
  for (let round = 0; round < 4; round++) {
    ...
    // 早期打ち切りは「確定的な成功」シグナルに限定する。確認画面では本文(textarea)に
    // 挨拶文「ありがとうございます」「後日担当者よりご連絡」等の弱い成功語が写り込み、
    // isSuccessContent が誤一致して最終ボタンを押さず終了してしまうため (例: tptc.co.jp)。
    if (
      (isStrongSuccess(content) || looksLikeSuccessUrl(page.url())) &&
      !(await onConfirmPage(page))
    )
      break;

    // 確認ボタンの出現待ちは長めに取る (遅延表示・JS再描画のフォーム対策。例: web.gogo.jp)。
    // CAPTCHA があるページではさらに待ってから探す (解決前にボタンが無効化されることがある)。
    const captchaHere = await hasCaptchaOnPage(page);
    const btn = await waitForConfirmationButton(page, captchaHere ? 20_000 : 12_000);
    if (!btn) break;

    // ユーザー要件: 確認/次画面に再び CAPTCHA が出ている場合は、解決の時間を確保して
    // トークンを注入してから次の送信ボタンを押す。reCAPTCHA は最大 60 秒待つ。
    // CAPTCHA が無ければ startCaptchaSolve は null を返すので何もしない。
    try {
...
async function onConfirmPage(page: Page): Promise<boolean> {
  return await page
    .evaluate(() => {
      ...
      if (
        /入力(?:画面|内容)?(?:へ|に)戻る|(?:内容|入力)を(?:修正|変更)|前の?(?:画面|ページ)に戻る/.test(txt)
      )
        return true;
      // 「戻る」フォームボタンと「送信/登録/申込/確定」ボタンが同居する典型的な確認画面。
      // (例: tptc.co.jp は最終ボタンが「登録」かつ「戻る」が並ぶ。ナビの戻るリンクと区別する
      //  ため、フォーム要素 button/input のみを対象にする。)
      const btns = Array.from(
        document.querySelectorAll("button, input[type=submit], input[type=button]"),
      );
      const labelOf = (e: Element) =>
        (((e as HTMLInputElement).value || "") + " " + (e.textContent || "")).trim();
      const hasBack = btns.some((e) => /戻る|もどる|back/i.test(labelOf(e)));
      const hasSubmit = btns.some((e) => {
        const l = labelOf(e);
        return (
          /送\s*信|登\s*録|申\s*込|申し込|確\s*定|送信する|submit|send/i.test(l) &&
          !/戻る|もどる|キャンセル|cancel|クリア|clear|リセット|reset|修正|訂正/i.test(l)
        );
      });
      return hasBack && hasSubmit;
    })
    .catch(() => false);
}
...
// ページ上に reCAPTCHA / hCaptcha / Turnstile が存在するか (ソルバの可否に依存しない検出)。
// TIMEOUT 等を「送信不可(CAPTCHA)」に再分類するためのシグナルに使う。
async function hasCaptchaOnPage(page: Page): Promise<boolean> {
  return await page
    .evaluate(() =>
...
async function runSubmit(
  ...
  // タイムアウト時の再分類用に CAPTCHA 検出状況を保持する (core 内で更新)。
  ...
  try {
    return await withDeadline(page, options, overallMs, stageRef, core, captchaRef);
  } finally {
...
export async function extractFormSnapshot(
  ...
  return await page.evaluate(() => {
    ...
    for (const el of Array.from(document.querySelectorAll("input, select, textarea"))) {
      ...
      if (!label) {
        // 1セル内に複数入力 (姓[input]名[input] 等) のレイアウト: 直前の兄弟を短いラベルに採用。
        let sib: ChildNode | null = el.previousSibling;
        for (let k = 0; sib && k < 4 && !label; k++) {
          let t = "";
          if (sib.nodeType === 3) t = sib.textContent || "";
          else if (sib.nodeType === 1 && !(sib as Element).querySelector("input, select, textarea"))
            t = sib.textContent || "";
