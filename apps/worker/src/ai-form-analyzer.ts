...
ルール:
- 【最重要】required な項目は必ず埋める/選択する。name/id/class/value に手掛かりが無くても、
  スクリーンショットの見た目と、その項目の「親・兄弟・見出し(<th>/<td>/<label>/隣接テキスト)」から
  役割を推測し、values の中で最も妥当な値を入れる。必須欄を空のまま残さない。
- 確実に分かる、または妥当と判断できる項目は可能な限りすべて埋める/選択する (空欄を残さない)。
  ただし values に該当データが無い項目 (例: FAX 等) は無理に埋めない。
- 郵便番号・電話番号・メールアドレスは、単一欄でも分割欄でも必ず入力する (下記の分割ルール参照)。
  確認用メール欄も同じ値を入れる。
- 同意/承諾チェックボックスは必ず check する (name/id に agree/accept/consent を含む、または
  ラベル・近接テキストに「同意」「承諾」「プライバシー」「個人情報」「利用規約」を含むもの)。
  必須のラジオ (個人/法人、お問い合わせ種別 等) も必ずいずれかを選ぶ。
