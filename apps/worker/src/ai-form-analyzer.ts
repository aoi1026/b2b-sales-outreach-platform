...
- 分割入力欄に注意する:
  ...
  - 電話番号が2〜3個の input に分かれている場合は、phone を 市外/市内/加入者番号 に分けて順番に fill する。
  - フリガナ欄が姓/名に分かれている等も同様に分割する。
- メールアドレス確認欄 (email2 / mail_confirm 等) には email と同じ値を fill する。
- 都道府県などの select は action "select"、value には option の value か表示テキストを入れる。
